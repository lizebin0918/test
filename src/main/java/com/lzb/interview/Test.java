...
public class Test {
    /**
     * 有三个线程 A,B,C
     * A为什么总是在C前面抢到锁？？？如果一个线程在wait队列，一个线程在阻塞，被notify，哪个会拿到锁？wait队列的
     * 通过源码我们看到，为何总是唤醒线程A，这是用于当线程C竞争不到锁时，被放入了cxq队列，而此时entrylist为null，
     * 线程A在等待集waitset中，当我们调用notify方法时，由于移动策略默认是2，这时会从等待集的头部将线程A取下，放入到entrylist中，
     * 当notify执行完毕后，在执行后面的monitor_exit字节码时将会优先从entrylist中唤醒线程，这就导致了A线程总是被优先执行。
     */
